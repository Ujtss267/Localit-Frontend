...
import type { EventCardPrettyProps } from "./types";
...
/** Mobile-first 이벤트 카드 패턴 (Localit 전용 톤) */
export default function EventCardPretty({
  ...
  return (
    <Card className={`group relative overflow-hidden ${className}`}>
      ...
      <p className="mt-3 text-[13px] sm:text-sm text-neutral-800 dark:text-neutral-200 line-clamp-2">
        {e.description}
      </p>
      ...
      <div className="mt-4 flex items-center justify-between">
        <Link
          to={toBuilder(e)}
          className="text-[13px] sm:text-sm font-medium text-blue-700 hover:underline dark:text-blue-300"
        >
          자세히 보기 →
        </Link>
